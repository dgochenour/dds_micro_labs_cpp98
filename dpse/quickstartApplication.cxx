...
// LAB - replace the dpde header with the dpse one
...
Long count)
{
    ...
    // LAB - use the DPSE::DiscoveryPluginProperty 
    DPSE::DiscoveryPluginProperty dpse_properties;
    ...
    *udp_property->allow_interface.get_reference(0) = String_dup("lo");
    ...
    // LAB -- register the dpse plugin
    if (!registry->register_component(
            "dpse",
            DPSE::DiscoveryFactory::get_interface(),
    ...
    // LAB -- set the discovery name in the DPF's QoS
    if (!dp_qos.discovery.discovery.name.set_name("dpse"))
    {
    ...
    *dp_qos.discovery.initial_peers.get_reference(0) = DDS_String_dup(peer);
    ...
    // LAB -- set the name of the "local" DP... this is required for DPSE
    ...
    // LAB -- for DPSE, we need to assert the remote participant(s) that we will discover
    retcode = DPSEDiscoveryPlugin::RemoteParticipant_assert(this->participant, remote_participant_name);
...
Application::~Application()
{
    ...
    // LAB -- update destructor to dpse
    if (!registry->unregister("dpse", NULL, NULL))
    {
