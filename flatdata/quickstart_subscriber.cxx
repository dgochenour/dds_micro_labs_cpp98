...
ImageReaderListener::on_data_available(DDSDataReader * reader)
{
    ...
    for (i = 0; i < sample_seq.length(); ++i)
    {
        ...
        if (sample_info->valid_data)
        {
            ...
            // LAB -- get the offsets of the root and other struct members
            ImageOffset offset_root = sample_seq[i].root();
            DimensionOffset dimension_offset = offset_root.dimension();
            rti::flat::PrimitiveArrayOffset<DDS_Octet, (30000000)> offset_img_data = offset_root.img_data();
            DDS_Octet *octet_array = rti::flat::plain_cast(offset_img_data);

            // LAB -- now we can access the data
            std::cout << "timestamp = " << offset_root.timestamp() << std::endl;
            std::cout << "dimension.width = " << dimension_offset.width() << std::endl;
            std::cout << "dimension.height = " << dimension_offset.height() << std::endl;
            std::cout << "img_data[0] = " << (int)octet_array[0] << std::endl;
            std::cout << "img_data[1] = " << (int)octet_array[1] << std::endl;
