...
DDS_Long count)
{
    ...
    NETIO::SHMEM::InterfaceFactoryProperty *shmem_property_cxx = NULL; 
    ...
    *udp_property->allow_interface.get_reference(0) = DDS_String_dup("lo");
    ...
    /* First configure the Domain Participant use the shared memory memory 
    * transport. References to shared memory segment are sent over the 
    * shared memory transport.
    *  
    * NOTE: The order of the enabling of the transports matters as this order 
    * determines preference when a sample can be sent using both 
    * transport. For instance, consider if shared memory transport is enabled 1st, 
    * and the UDP transport is enabled 2nd. If a receiving endpoint can receive
    * across shared memory and UDP, the middleware will only send samples
    * over shared memory. If the UDP transport is enabled 1st and the
    * shared memory segment is enabled 2nd, then in the same scenario the 
    * middleware will send a sample across the UDP transport
    */     
    ...
    *dp_qos.transports.enabled_transports.get_reference(1) = DDS_String_dup("_udp");
    ...
    *dp_qos.discovery.enabled_transports.get_reference(1) = DDS_String_dup("_udp://");
    ...
    *dp_qos.user_traffic.enabled_transports.get_reference(1) = DDS_String_dup("_udp://");

    dp_qos.discovery.initial_peers.maximum(2);
    dp_qos.discovery.initial_peers.length(2);
    *dp_qos.discovery.initial_peers.get_reference(0) = DDS_String_dup("_shmem://");
    *dp_qos.discovery.initial_peers.get_reference(1) = DDS_String_dup(peer);
    ...
    strcpy(this->topic_name, "Example Image");
